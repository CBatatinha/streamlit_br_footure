...
def tabela_defesa(df):
    ...
    desarmes_defesa=desarmes.query('x<=30').reset_index(drop=True)
    desarmes_meio=desarmes.query('(x>30)&(x<=70)').reset_index(drop=True)
...
def tabela_posse(df):
    ...
    toques_defesa=toques.query('x<=30').reset_index(drop=True)
    toques_meio=toques.query('(x>30)&(x<=70)').reset_index(drop=True)
